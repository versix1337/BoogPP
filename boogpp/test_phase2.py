#!/usr/bin/env python3
"""
Integration tests for Boogpp Phase 2 components:
- Type Checker
...
"""
    ...
    try:
        tokens = tokenize(code, "test_basic.bpp")
        ast = parse(tokens)
...
"""
    ...
    try:
        tokens = tokenize(code, "test_inference.bpp")
        ast = parse(tokens)
        errors = check_types(ast)
...
"""
    ...
    try:
        tokens = tokenize(code, "test_errors.bpp")
        ast = parse(tokens)
        errors = check_types(ast)
...
"""
    ...
    try:
        tokens = tokenize(code, "test_safety.bpp")
        ast = parse(tokens)
...
"""
    ...
    try:
        tokens = tokenize(code, "test_codegen.bpp")
        ast = parse(tokens)
...
"""
    ...
    try:
        tokens = tokenize(code, "test_control_flow.bpp")
        ast = parse(tokens)
...
"""
    ...
    try:
        ...
        tokens = tokenize(code, "test_pipeline.bpp")
...
def run_all_tests():
    """Run all tests"""
    print("\n" + "=" * 60)
    print("Boogpp Phase 2 Integration Tests")
    print("=" * 60)
