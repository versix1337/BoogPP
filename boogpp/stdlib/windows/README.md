# Boogpp Windows API Bindings

This directory contains Windows API bindings for Boogpp, providing direct access to Windows system calls with minimal boilerplate.
...
Boogpp provides system hooks that intercept Windows events:
...
@service(
    name: "MyService",
    display_name: "My Boogpp Service",
    description: "A service written in Boogpp",
    start_type: AUTO,
    run_as: SYSTEM
)
...
```bash
boogpp build myapp.cos -o myapp.exe
```

### Compile to Windows service:
```bash
boogpp build myservice.cos --type service -o myservice.exe
```

### Compile to kernel driver:
```bash
boogpp build mydriver.cos --type driver -o mydriver.sys --safety unsafe
```
