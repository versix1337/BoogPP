"""
CustomOS LLVM IR Code Generator
...
class LLVMCodeGenerator:
    ...
    def generate(self, program: Program, type_annotations: Optional[Dict[ASTNode, Type]] = None) -> str:
        ...
        self._emit(f'source_filename = "{self.module_name}.cos"')
        self._emit('')
    ...
    def _get_llvm_type(self, typ: Type) -> str:
        """Convert CustomOS type to LLVM type string"""
    ...
    def generate_builtin_call(self, func_name: str, args: List[Expression]) -> str:
        ...
        if func_name == "print":
            if args:
                arg_reg = self.generate_expression(args[0])
                self._emit(f"call void @print(i8* {arg_reg})")
            return "0"
